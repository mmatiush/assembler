...
/*   Updated: 2018/10/26 14:51:47 by mmatiush         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned short	swap_short_bytes(unsigned short n)
{
	unsigned short res;
	res  = (n >> 8) | (n << 8);
	return (res);
...
void		write_header_to_output_file(t_header *header, int fd)
{
	...
	if (!is_big_endian())
	{
		header->magic = swap_int_bytes(header->magic);
		header->prog_size = swap_int_bytes(header->prog_size);
	}
	...
	while (op->param_size[i] && i < NEW_MAX_OP_ARGS_NUMBER)
	{
		...
		if (!is_big_endian())
		{
			if (op->param_size[i] == 2)
				op->param[i] = swap_short_bytes((unsigned short)op->param[i]);
			if (op->param_size[i] == 4)
				op->param[i] = swap_int_bytes(op->param[i]);
		}
